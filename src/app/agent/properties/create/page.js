...
export default function AgentCreateProperty() {
    ...
    const handleFileChange = async (e) => {
        ...
        if (currentCount + files.length > 5) {
            showToast(`Maximum 5 images total. You already have ${currentCount} selected.`, "error");
            return;
        }
    ...
    const uploadImages = async (propertyId, token) => {
        ...
        try {
            ...
            if (res.ok || data.code === 200 || data.status === "success") {
                ...
                showToast(`Images upload failed: ${data.message || "unknown error"}`, "error");
            }
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (imageFiles.length > 5) {
            showToast("Maximum 5 images allowed. Please remove some images.", "error");
            return;
        }
        ...
        try {
            ...
            if (res.ok || data.code === 200 || data.code === 201 || data.status === "success") {
                const propertyId =
                ...
                if (propertyId) {
                    ...
                    showToast("Property created but image upload skipped — ID not found.", "error");
                }
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                                {[
                                    ...
                                    <div key={f.name}>
                                        ...
                                        <input type="number" name={f.name} value={formData[f.name]} onChange={handleNumberChange} className={inputClass} min="0" />
                                    </div>
                                ))}
                                <div>
